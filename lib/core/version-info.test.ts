...
describe('buildSummary', () => {
  test('is one line naming the app, build and platform', () => {
    assert.equal(buildSummary(INFO), 'PasarWhere 1.0.0 (4) · iOS 18.5');
  });

  test('Android reports its API level, which is what Platform.Version gives there', () => {
    assert.equal(buildSummary({ ...INFO, os: 'android', osVersion: 35 }), 'PasarWhere 1.0.0 (4) · Android 35');
  });
});
...
describe('feedbackUrl', () => {
  test('quotes the build summary below a blank writing area', () => {
    assert.equal(
      ...
      'mailto:t@sfdye.com?subject=PasarWhere%20feedback&body=%0A%0A%E2%80%94%0APasarWhere%201.0.0%20(4)%20%C2%B7%20iOS%2018.5'
    );
  });
});
